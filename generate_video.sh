...
function escape_ffmpeg_metadata (){
  echo -e "$1" |sed -nE 's/([=;#\\])/\\\0/g;H;1h;${g;s/\n/\\n/g;p}'
}
...
select ep.number, ep.slug, coalesce(ep.title,po.post_title), date_format(ep.recording_date,"%Y-%m-%dT%H:%i:%sZ"), date_format(po.post_date,"%Y-%m-%dT%H:%i:%sZ"), kw.kw, ep.summary, se.links
	...
	left join (select sep.id id, group_concat(concat(se.title,": ",se.original_url) SEPARATOR "\\n") as links from ${MYSQL_PREFIX}podlove_modules_shownotes_entry se, ${MYSQL_PREFIX}podlove_episode sep where sep.id=se.episode_id group by sep.id) se on se.id=ep.id 
        ...
        where ep.number is not NULL and po.post_status not in ("auto-draft","inherit","trash")
EOM
...
function get_temp_image() {
    ...
    local ext=$(file ${episode_tmp} --extension|cut -d: -f2|cut -d/ -f1|tr -d " ")
...
function create_images () {
	...
	echo -e ";FFMETADATA1\ntitle=$(escape_ffmpeg_metadata "${EPISODE_TITLE}")\ndecription=$(escape_ffmpeg_metadata "${EPISODE_SUMMARY}")\nshow=$(escape_ffmpeg_metadata "${PODCAST_TITLE}")\nepisode_id=$EPISODE\n\n" > ${CHAPTER_METADATA}
    fi
	...
	case "$k" in
		...
		if [ ! -z "$prev_title" ]; then
  		    echo -e "[CHAPTER]\nTIMEBASE=1/1000\nSTART=${last_position}000\nEND=${position}000\ntitle=$(escape_ffmpeg_metadata "${prev_title}")\n" >>$CHAPTER_METADATA
		fi
		;;
	    "image" )
	...
	if ( [ "$k" = "title" ] && [ "${last_title}" != "${v}" ] ) || [ ! -z "${done_last}" ]; then
		    local duration=$(( $position - $last_position ))
    		    if [ ! -z "${image}" ]; then
		    	local temp_image=$(get_temp_image "${image}")
		    fi
		    if [ ! "${skip_image_frames}" = "true" ]; then 
		    	mv $(create_chapter_frame "${last_chapter_frame}" "${last_title}" "${temp_image}") "${TARGET_DIR}/images/${EPISODE_SLUG}/${EPISODE_SLUG}_${position}.jpg"
			...
			"/>" >> "${psc_out}"
		    if [ ! "${skip_video_frames}" = "true" ]; then 
		    	last_video_frame=$(add_episode_frame "$SCRIPT" "$duration" "$last_video_frame" "$last_title" "$temp_image")
		    fi
		    [ ! -z "${temp_image}" ] && rm "${temp_image}"
		    local last_position=$position
		    local href=""
		    local image=""
		    local prev_title="$last_title"
...
function create_video () {
    ...
    local mp3src="${TARGET_DIR}/${EPISODE_SLUG}.mp3"
    local ff_params="-hide_banner -y -hwaccel vaapi" 
    local video_params="-c:v h264 -preset fast -tune stillimage -profile:v high -pix_fmt yuvj420p -async 1 -f mp4"
    local video_out_params="-fps_mode vfr -vf fps=24 -b:v 500K"
#    local video_out_params="-crf 18"
